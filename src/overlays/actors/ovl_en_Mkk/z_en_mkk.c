...
void func_80A4E2E8(EnMkk* this, PlayState* play) {
    ...
    if ((player->stateFlags2 & PLAYER_STATE2_7) || (player->actor.freezeTimer > 0)) {
    ...
    func_8002F974(&this->actor, NA_SE_EN_AMOS_DAMAGE - SFX_FLAG); // TODO : Change SFX
    if (sp20) {
        this->unk_14B &= ~2;
        func_80A4E190(this);
    } else if ((this->unk_149 == 0) && (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) &&
...
void func_80A4E58C(EnMkk* this) {
    osSyncPrintf("Enter 3\n");

    this->unk_14B |= 1;
    this->actor.speed = 3.0f;
    this->actor.velocity.y = 5.0f;
    Actor_PlaySfx(&this->actor, NA_SE_EN_ANUBIS_FIRE); // TODO : Change SFX
...
void func_80A4E67C(EnMkk* this) {
    osSyncPrintf("Enter 5\n");

    this->unk_14B |= 1;
    this->actor.flags &= ~ACTOR_FLAG_0;
    this->collider.base.acFlags &= ~AC_ON;
    this->actor.flags |= ACTOR_FLAG_2;
...
void EnMkk_Update(Actor* thisx, PlayState* play) {
    ...
    osSyncPrintf("actionFunc\n");

    this->actionFunc(this, play);

    osSyncPrintf("actionFunc 2\n");
    ...
    osSyncPrintf("CollisionCheck_SetOC\n");
